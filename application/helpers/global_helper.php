<?php
	...
	if(!function_exists('load_assets')) {
		function load_assets($assets,$type){
			$ci =& get_instance();
			echo "\r\n";
			foreach ($assets[$ci->router->fetch_class()][$type] as $key => $value) {
				if($type == 'css'){									
					echo "<link rel='stylesheet' href='". base_url('assets/' . $ci->router->fetch_class() . '/'.$type.'/') . $value . "' />";
				} else {					
					echo "<script src='" . base_url('assets/' . $ci->router->fetch_class() . '/' . $type . '/') . $value . "' ></script>";
				}
				echo "\r\n";
			}
	    }
